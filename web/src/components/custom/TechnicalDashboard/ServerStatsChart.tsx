    ...
    return (
        <SimpleCard>
            ...
            <div className="mt-4 text-white">
                <h5 className="font-bold my-2">System-Ressources</h5>
                <table className="w-full border-collapse border border-borders text-sm">
                    <thead>
                        <tr className="bg-cards">
                            <th className="border border-borders p-2">Ressource</th>
                            <th className="border border-borders p-2">Total (MiB)</th>
                            <th className="border border-borders p-2">Used (MiB)</th>
                            <th className="border border-borders p-2">Free (MiB)</th>
                        </tr>
                    </thead>
                    <tbody>
                        {data?.totalMemoryMiB && data?.freeMemoryMiB && (
                            <tr>
                                <td className="border border-borders p-2">Memory (RAM)</td>
                                <td className="border border-borders p-2">{data.totalMemoryMiB}</td>
                                <td className="border border-borders p-2">{(data.totalMemoryMiB - data.freeMemoryMiB).toFixed(0)}</td>
                                <td className="border border-borders p-2">{data.freeMemoryMiB}</td>
                            </tr>
                        )}
                        {data?.totalSwapMiB && data?.freeSwapMiB && (
                            <tr>
                                <td className="border border-borders p-2">Swap-RAM</td>
                                <td className="border border-borders p-2">{data.totalSwapMiB}</td>
                                <td className="border border-borders p-2">{(data.totalSwapMiB - data.freeSwapMiB).toFixed(0)}</td>
                                <td className="border border-borders p-2">{data.freeSwapMiB}</td>
                            </tr>
                        )}
                        {data?.totalDiskSpaceMiB && data?.freeDiskSpaceMiB && (
                            <tr>
                                <td className="border border-borders p-2">Storage</td>
                                <td className="border border-borders p-2">{data.totalDiskSpaceMiB}</td>
                                <td className="border border-borders p-2">{(data.totalDiskSpaceMiB - data.freeDiskSpaceMiB).toFixed(0)}</td>
                                <td className="border border-borders p-2">{data.freeDiskSpaceMiB}</td>
                            </tr>
                        )}
                    </tbody>
                </table>
                <div className="mt-4 font-bold">Uptime: {formatUptime(data?.systemUptimeSeconds || 0)}</div>
            </div>
        </SimpleCard>
    );
};
